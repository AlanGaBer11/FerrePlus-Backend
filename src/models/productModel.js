...
const getAllProducts = async () => {
  try {
    const result = await db.query(
      "SELECT p.id_product, p.name, p.category, p.price, p.stock, s.name AS supplier_name FROM products p JOIN suppliers s ON p.id_supplier = s.id_supplier ORDER BY p.id_product ASC"
    );
    return result.rows;
  } catch (err) {
