const supplierProcess = require("../processes/supplierProcess");
...
const getSupplierById = async (req, res) => {
  try {
    const supplier = await supplierProcess.getSupplierById(req.params.id);
    if (!supplier) {
      return res
        .status(404)
...
const deleteSupplier = async (req, res) => {
  try {
    const deletedSupplier = await supplierProcess.deleteSupplier(req.params.id);
    if (!deletedSupplier) {
      return res
        .status(404)
        .json({ succes: false, message: "Proveedor no encontrado" });
